...
//	SetupValue(Vars.Visuals.Radar.Y, 500, strenc("Visuals"), strenc("RadarY"));
	...
	SetupValue(Vars.Visuals.thirdpersonkey, 0, strenc("Visuals"), strenc("ThirdPerson.Key"));
	SetupValue(Vars.Visuals.disablepost.enabled, false, strenc("Visuals"), strenc("Disable.Post"));
	SetupValue(Vars.Visuals.nightmode.enabled, false, strenc("Visuals"), strenc("NightMode"));
	SetupValue(Vars.Visuals.trace.enabled, false, strenc("Visuals"), strenc("trace"));
	...
	Vars.Misc.ClantagChanger.value = strdup("");
...
void CConfig::Save()
{
	...
	{
		char szCmd[256];
		sprintf(szCmd, "\\trapware\\%s.ini", Vars.Misc.configname);

		folder = std::string( path ) + strenc( "\\trapware\\" );
